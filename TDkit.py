...
consumer_key=open('C:\Python\API Keys\TD\TD_CONSUMER_KEY.txt', 'r').read()
redirect_uri='http://localhost'
credentials_path='Credential.json'
api_key=open('C:\Python\API Keys\TD\TD_ACCESS_TOKEN.txt', 'r').read()
td_client=TDClient(client_id=consumer_key, redirect_uri=redirect_uri, credentials_path=credentials_path)    

Ticker_List={'Tickers':['AAPL',
                        'NVDA', 
                        'AMD',
                        'AMZN',
                        'TSLA']}
...
def Closing_price_alltime(Ticker, n_years, limit_speed=False):
    """
    Gets Closing Price for Past 20 Years w/ Daily Intervals
    and Formats it to correct Date and single 'Closing Price'
    column.  limit_speed must be enabled if number of calls
    is > 120.
    """
...
def Port_consol(Ticker_List, n_years, limit_speed=False):
    """
    ...
    for Ticker in Ticker_List['Tickers']:
        try:
            Port.append(Closing_price_alltime(Ticker, n_years, limit_speed))
        except Exception:
            pass
    j=list(range(0, len(Port)))
    for i in j:
        data = operator.itemgetter('datetime','close')
        if Port[i]['empty']==False:
...
def yf_historical_close(ticker_list, start, end, interval='1d', auto_adjust=False, actions=False):
    """
    Pulls Historical Data from Yahoo Finance
    using a List of Tickers
    """
    Data=[]
    for Ticker in ticker_list['Tickers']:
        yf_Ticker=yf.Ticker(Ticker)
        Data.append({Ticker : yf_Ticker.history(interval=interval, start='2020-01-02', end='2020-12-31', auto_adjust=auto_adjust, actions=actions)})
    Data2=[]
    j=list(range(0, len(Data)))
    for i in j:
        for key in Data[i]:
            symbol=key
            candles=Data[i][key]['Close']
            date=Data[i][key].index
            series=pd.Series(data=candles, index=date, name=symbol)
            Data2.append(series)
    historical_rets_df=pd.DataFrame(Data2).T
    return historical_rets_df 
...
def SPY_ann(n_years=20, vol=False):
    """
    ...
    if vol==True:
        SPY_ann_vol=annualize_vol(SPY_pct, periods_per_year=n_mkt_days_yr)
        return SPY_ann_vol
    else:
...
def drawdown2(return_series: pd.Series):
    ...
    """
    cumulative_return = ((1+return_series).cumprod()-1)*100
    previous_peaks = cumulative_return.cummax()
    drawdowns = ((cumulative_return - previous_peaks)/previous_peaks)*100
    return pd.DataFrame({"Return": cumulative_return, 
